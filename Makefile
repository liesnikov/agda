...
v2-quicker-build:
	$(CABAL) build exe:agda $(CABAL_OPT_FAST) --builddir=$(QUICK_BUILD_DIR)-v2 -j1 --disable-library-profiling -fdebug -fenable-cluster-counting --ghc-options=$(GHC_OPTS) $(CABAL_OPTS)
	@echo "$(QUICK_BUILD_DIR)-v2/build/x86_64-linux/ghc-$(GHC_VER)/Agda-$(VERSION)/build/agda/agda"
